...
from info import AUTH_CHANNEL, LONG_IMDB_DESCRIPTION, MAX_LIST_ELM, SHORTLINK_URL, SHORTLINK_API, IS_SHORTLINK, LOG_CHANNEL, TUTORIAL, GRP_LNK, CHNL_LNK, CUSTOM_FILE_CAPTION, HOW_TO_VERIFY, OWNER_ID, ADMINS
...
def is_admin(user_id):
    ...
    return user_id in ADMINS and (not OWNER_ID or user_id != OWNER_ID)
...
def get_user_role(user_id):
    """Get user role: 'owner', 'admin', or 'user'"""
    if is_owner(user_id):
        return 'owner'
...
# Utility functions for system stats
def get_system_stats():
    try:
        ...
        total_memory = humanbytes(memory_info.total)
        available_memory = humanbytes(memory_info.available)

        # Disk Usage
        disk_info = psutil.disk_usage('/')
        disk_usage = disk_info.percent
        total_disk = humanbytes(disk_info.total)
        used_disk = humanbytes(disk_info.used)
        free_disk = humanbytes(disk_info.free)

        # Network Stats (can be complex, providing basic for now)
        # net_io = psutil.net_io_counters()
        # bytes_sent = humanbytes(net_io.bytes_sent)
        # bytes_recv = humanbytes(net_io.bytes_recv)

        # System Uptime
        start_time_obj = datetime.strptime(temp.START_TIME, "%Y-%m-%d %H:%M:%S")
        ...
        stats = (
            ...
            f"<b>💻 OS:</b> {system_os} {os_release} ({os_version})\n"
            f"<b>🚀 Processor:</b> {processor}\n"
            f"<b>⏳ Uptime:</b> {uptime}\n\n"
            f"<b>📈 CPU Usage:</b> {cpu_usage}%\n"
            f"<b>💾 Memory Usage:</b> {memory_usage}% ({available_memory}/{total_memory})\n"
            f"<b>💽 Disk Usage:</b> {disk_usage}% ({used_disk}/{total_disk} - Free: {free_disk})"
            # f"<b>🌐 Network Sent:</b> {bytes_sent}\n"
            # f"<b>🌐 Network Received:</b> {bytes_recv}"
        )
        return stats
    except Exception as e:
