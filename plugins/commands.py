...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        if OWNER_ID and user_id == OWNER_ID:
            ...
            buttons = [[
                        InlineKeyboardButton('➕ Add Me To New Group', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
                    ],[
                ...
                InlineKeyboardButton("⚙️ Setting", callback_data="admin_settings"),
                InlineKeyboardButton("📋 Logs", callback_data="get_logs")
            ]]
        elif user_id in ADMINS and (not OWNER_ID or user_id != OWNER_ID):
            # Admin Panel (Limited Control) - Only if not owner
            buttons = [[
        ...
        if OWNER_ID and user_id == OWNER_ID:
            welcome_message = script.OWNER_PANEL_TXT.format(message.from_user.mention, temp.U_NAME, temp.B_NAME)
        elif user_id in ADMINS:
            welcome_message = script.ADMIN_PANEL_TXT.format(message.from_user.mention, temp.U_NAME, temp.B_NAME)
        else:
            welcome_message = script.USER_PANEL_TXT.format(message.from_user.mention, temp.U_NAME, temp.B_NAME)
...
@Client.on_message(filters.command('settings') & filters.user(ADMINS + ([OWNER_ID] if OWNER_ID else [])))
...
@Client.on_message(filters.command('set_template') & filters.user(ADMINS + ([OWNER_ID] if OWNER_ID else [])))
...
@Client.on_message((filters.command(["request", "Request"]) | filters.regex("#request") | filters.regex("#Request")) & filters.group & filters.user(ADMINS + ([OWNER_ID] if OWNER_ID else [])))
...
@Client.on_message(filters.command("set_shortner") & filters.user(ADMINS + ([OWNER_ID] if OWNER_ID else [])))
...
@Client.on_message(filters.command("get_info") & filters.user(ADMINS + ([OWNER_ID] if OWNER_ID else [])))
...
@Client.on_message(filters.command("set_tutorial") & filters.user(ADMINS + ([OWNER_ID] if OWNER_ID else [])))
...
@Client.on_message(filters.command("remove_tutorial") & filters.user(ADMINS + ([OWNER_ID] if OWNER_ID else [])))
...
@Client.on_message(filters.command("ban") & filters.user(ADMINS + ([OWNER_ID] if OWNER_ID else [])))
async def ban_user(bot, message):
    ...
    try:
        ...
        try:
            await bot.send_message(user_id, f"❌ You have been banned from using this bot.\nReason: {reason}")
        except:
            pass
...
@Client.on_message(filters.command("unban") & filters.user(ADMINS + ([OWNER_ID] if OWNER_ID else [])))
async def unban_user(bot, message):
    ...
    try:
        ...
        await message.reply(f"✅ Successfully unbanned user `{user_id}`")
        
        try:
            await bot.send_message(user_id, "✅ You have been unbanned! You can now use the bot again.")
        except:
            pass
...
@Client.on_message(filters.command("banned") & filters.user(ADMINS + ([OWNER_ID] if OWNER_ID else [])))
async def list_banned_users(bot, message):
    banned_users, banned_chats = await db.get_banned()
    
    if not banned_users and not banned_chats:
        return await message.reply("✅ No banned users or chats found!")
    
    text = "**🚫 Banned Users & Chats**\n\n"
