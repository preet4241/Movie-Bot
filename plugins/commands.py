    ...
    if len(message.command) != 2:
        ...
        if user_id in OWNER:
            ...
            buttons = [[
                ...
                InlineKeyboardButton("🗂️ Files", callback_data="files_stats")
                ],[      
...
@Client.on_message(filters.command("send") & filters.user(OWNER))
...
async def shortlink(bot, message):
    ...
    if user.status != enums.ChatMemberStatus.ADMINISTRATOR and user.status != enums.ChatMemberStatus.OWNER and str(userid) not in OWNER:
...
@Client.on_message(filters.command("setshortlinkoff") & filters.user(OWNER))
...
@Client.on_message(filters.command("setshortlinkon") & filters.user(OWNER))
