...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import CHANNELS, ADMINS, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, SHORTLINK_URL, SHORTLINK_API, TUTORIAL, IS_TUTORIAL, ADMINS, PICS, NOR_IMG, MELCOW_VID, SPELL_IMG, NEWGRP_IMG, CLOSE_IMG, VERIFY, SHORTLINK_API, SHORTLINK_URL, TUTORIAL, IS_TUTORIAL, HOW_TO_VERIFY, MAX_BTN, languages, LANGUAGES, SEASONS, QUALITY, YEARS, IMDB_DELET_TIME, LOG_CHANNEL, SUPPORT_CHAT, REQST_CHANNEL, SUPPORT_CHAT_ID, AUTO_FFILTER, AUTO_DELETE, MAUTO_DELETE, SINGLE_BUTTON, LONG_IMDB_DESCRIPTION, SPELL_CHECK_REPLY, MAX_LIST_ELM, PROTECT_CONTENT, PUBLIC_FILE_STORE, NO_RESULTS_MSG, PM_FILE_DELETE_TIME, FILE_STORE, PM_IMDB, PM_SEARCH, PREMIUM_USER, OWNER_ID
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in [str(x) for x in ADMINS + ([OWNER_ID] if OWNER_ID else [])]):
            await del_all(query.message, grp_id, title)
        else:
            await query.answer("Yᴏᴜ ɴᴇᴇᴅ ᴛᴏ ʙᴇ Gʀᴏᴜᴘ Oᴡɴᴇʀ ᴏʀ ᴀɴ Aᴜᴛʜ Usᴇʀ ᴛᴏ Dᴏ Tʜᴀᴛ!", show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        if (
                ...
                and str(userid) not in [str(x) for x in ADMINS + ([OWNER_ID] if OWNER_ID else [])]
        ):
        ...
        if settings is not None:
            ...
            reply_markup = InlineKeyboardMarkup(buttons)
            await query.message.edit_text(
                f"<b>Cʜᴀɴɢᴇ Yᴏᴜʀ Sᴇᴛᴛɪɴɢs Fᴏʀ {title} As Yᴏᴜʀ Wɪsʜ ⚙</b>",
                disable_web_page_preview=True,
                parse_mode=enums.ParseMode.HTML
            )
        ...
        if query.from_user.id in ADMINS + ([OWNER_ID] if OWNER_ID else []):
        ...
        # Check if user is owner or admin for different help menu
        if query.from_user.id == OWNER_ID or query.from_user.id in ADMINS:
            buttons = [[
            ...
            # User panel help with updated format
            buttons = [[
                InlineKeyboardButton('ᴀɴʏ ǫᴜᴇʀʏ ✉️', url='t.me/kissuhelp'),
                InlineKeyboardButton('∙ ʀᴜʟᴇs ∙', callback_data='rule_btn')
            ], [
    ...
    [
        ...
        if query.from_user.id == OWNER_ID or query.from_user.id in ADMINS:
            buttons = [[
                ...
                InlineKeyboardButton("Give Credit", callback_data="give_credit_menu"),
...
**⏱️ System Info:**
• Uptime: `{uptime_str}`
• Bot Started: `{temp.START_TIME}`

**📡 Network:**
• Bot Username: @{temp.U_NAME}
...
"""
            ...
            await query.answer("Only owner can view advanced bot statistics!", show_alert=True)

    elif query.data == "ban_user_menu":
        await query.message.edit_text(
            "**Ban User**\n\nSend the user ID or forward a message from the user you want to ban.",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("Back", callback_data="users_stats")
            ]])
        )

    elif query.data == "unban_user_menu":
        await query.message.edit_text(
            "**Unban User**\n\nSend the user ID of the user you want to unban.",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("Back", callback_data="users_stats")
            ]])
        )

    elif query.data == "give_credit_menu":
        await query.message.edit_text(
            "**Give Credit**\n\nSend the user ID and credit amount in format:\n`user_id amount`\n\nExample: `12345 100`",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("Back", callback_data="users_stats")
            ]])
        )

    elif query.data == "give_credit_all":
        await query.message.edit_text(
            "**Give Credit to All Users**\n\nSend the credit amount to give to all users:",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("Back", callback_data="users_stats")
            ]])
        )
        ...
        info_text = f"""**👥 Users Information**

**Total Users:** `{total_users}`
**Banned Users:** `{len(banned_users)}`
**Banned Chats:** `{len(banned_chats)}`

**Recent Activity:**
• Active Users (Last 24h): `Coming Soon`
• New Users (Last 7 days): `Coming Soon`
"""
        ...
        if set_type == 'is_shortlink' and query.from_user.id not in ADMINS + ([OWNER_ID] if OWNER_ID else []):
...
@Client.on_message(filters.command(['imdb', 'search']) & filters.user(ADMINS + ([OWNER_ID] if OWNER_ID else [])))
async def imdb_search(client, message):
    try:
        key = message.text.strip().split(" ", maxsplit=1)[1]
    except IndexError:
        await message.reply("Please provide a movie name or query to search.\nExample: `/imdb The Dark Knight`")
        return
    await imdb_search_command(client, message, key)

@Client.on_message(filters.command("search") & filters.user(ADMINS + ([OWNER_ID] if OWNER_ID else [])))
async def search_imdb(client, message):
    try:
        ...
        return
    await search_command(client, message, key)

@Client.on_message(filters.command("id"))
async def give_id(client, message):
    if message.chat.type == enums.ChatType.PRIVATE:
        ...
        await message.reply_text("I can't recognize this chat type.")
    ...
    await message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton(
                        " Make Your Own Bot", url="https://GitHub.com/pykinsu/tele-filter-bot/"
                    )
                ],
            ]
        ),
...
async def filters_cmd(client, message):
    await message.reply_photo(
        photo=random.choice(PICS),
        caption=script.ALL_FILTERS.format(message.from_user.mention),
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("Mᴀɴᴜᴀʟ FIʟᴛᴇʀ", callback_data="manuelfilter"),
...
async def settings_cmd(client, message):
    ...
    if grpid:
        await message.reply_photo(
            photo=random.choice(PICS),
            caption=f"<b>Cʜᴀɴɢᴇ Yᴏᴜʀ Sᴇᴛᴛɪɴɢs Fᴏʀ {message.chat.title} As Yᴏᴜʀ Wɪsʜ ⚙</b>",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "Cʜᴀɴɢᴇ Sᴇᴛᴛɪɴɢs", callback_data=f"opnsetgrp#{grpid}"
                        ),
                        InlineKeyboardButton("Gʀᴏᴜᴘ Sᴇᴛᴛɪɴɢs", callback_data=f"opnsetpm#{grpid}"),
                    ],
                    [
                        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help")
                    ],
        ...
        await message.reply_text(
            "Yᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴄᴏɴɴᴇᴄᴛᴇᴅ ᴛᴏ ᴀɴʏ ɢʀᴏᴜᴘs!\nCʜᴇᴄᴋ /connections ᴏʀ ᴄᴏɴɴᴇᴄᴛ ᴛᴏ ᴀɴʏ ɢʀᴏᴜᴘs",
            quote=True,
        )
...
async def connections_cmd(client, message):
    ...
    if buttons:
        await message.reply_photo(
            photo=random.choice(PICS),
            caption="Yᴏᴜʀ ᴄᴏɴɴᴇᴄᴛᴇᴅ ɢʀᴏᴜᴘ ᴅᴇᴛᴀɪʟs ;\n\n",
            reply_markup=InlineKeyboardMarkup(buttons),
            parse_mode=enums.ParseMode.HTML,
        )
    else:
...
async def check_verification(client, user_id):
    if int(user_id) in PREMIUM_USER:
        return True
    if not IS_VERIFY:
        return True
    ...
    if not res:
        return False
    verified_date = res.get("verified_date")
    if verified_date is None:
        return False
    date_diff = datetime.today() - datetime.strptime(verified_date, "%Y-%m-%d").replace(tzinfo=pytz.timezone('Asia/Kolkata'))
    return date_diff.days < 1

async def get_token(client, user_id, start_link, file_id):
    token = await db.get_token(user_id)
    if not token:
        token = await db.create_token(user_id)
    return f"{start_link}verify_{token}"

async def imdb_search_command(client, message, key):
    reply_message = await message.reply_text(
        f"<code>Searching for {key}...</code>", quote=True
    )
    try:
        movie = await get_poster(key, file=(await client.get_messages(message.chat.id, message.reply_to_message.id)).video.file_name if message.reply_to_message else None)
        if movie:
            await reply_message.edit_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                text="IMDb Link", url=movie.split("Link: ")[1].split("\n")[0]
                            )
                        ]
                    ]
                ),
            ...
            await reply_message.edit_text(f"No results found for <b>{key}</b>.")
    except Exception as e:
        logger.exception(e)
        await reply_message.edit_text(f"An error occurred: {e}")

async def search_command(client, message, key):
    reply_message = await message.reply_text(
        f"<code>Searching for {key}...</code>", quote=True
    )
    try:
        files, offset, total_results = await get_search_results(message.chat.id, key, offset=0, filter=True)
        if files:
            settings = await get_settings(message.chat.id)
            pre = 'filep' if settings['file_secure'] else 'file'
            btn = [
                ...
                for file in files[:settings.get('max_btn', 10)]
            ]
            btn.append([InlineKeyboardButton(text="NEXT ➪", callback_data=f"next_{message.from_user.id}_{message.chat.id}-{message.id}_{offset}")])
            await reply_message.edit_text(
                f"Found {total_results} results for {key}.",
                reply_markup=InlineKeyboardMarkup(btn)
            )
        else:
            await reply_message.edit_text(f"No results found for <b>{key}</b>.")
